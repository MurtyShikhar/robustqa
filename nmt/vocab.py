...
Usage:
    vocab.py --src-lang=<string> --tgt-lang=<string> --src-train-vocab-size=<int> --tgt-train-vocab-size=<int> [options]

Options:
    -h --help                  Show this screen.
    --src-lang=<string>         source language
    --tgt-lang=<string>         target language
    --size=<int>               vocab size [default: 50000]
    --freq-cutoff=<int>        frequency cutoff [default: 2]
    --src-train-vocab-size=<int>      source langauge training vocab size
    --tgt-train-vocal-size=<int>      tgt lang training vocab size
"""
...
from utils import read_corpus, pad_sents, gen_file_path
...
def get_vocab_list(file_path, save_dir, source, vocab_size):
    ...
    @save_dir: directory to save model and vocab
    """
    spm.SentencePieceTrainer.train(input=file_path, model_prefix=os.path.join(save_dir, source), vocab_size=vocab_size)     # train the spm model
    sp = spm.SentencePieceProcessor()                                                               # create an instance; this saves .model and .vocab files 
    sp.load(os.path.join(save_dir, '{}.model'.format(source)))                                      # loads tgt.model or src.model
...
if __name__ == '__main__':
    ...
    src_sents = get_vocab_list(train_src, save_dir, source='src', vocab_size=args['--src-train-vocab-size'])
    tgt_sents = get_vocab_list(train_tgt, save_dir, source='tgt', vocab_size=args['--tgt-train-vocab-size'])

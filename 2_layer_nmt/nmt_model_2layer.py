...
class NMT(nn.Module):
    ...
    def __init__(self, embed_size, hidden_size, vocab, dropout_rate=0.2):
        ...
        # bias by default is True
        self.encoder = nn.LSTM(input_size=embed_size, hidden_size=self.hidden_size, bidirectional=True, num_layers=2)
        ...
        self.combined_output_projection = nn.Linear(in_features=3*self.hidden_size, out_features=self.hidden_size,
                                                    bias=False)
        self.target_vocab_projection = nn.Linear(in_features=self.hidden_size, out_features=len(self.vocab.tgt),
                                                 bias=False)
    ...
    def encode(self, source_padded: torch.Tensor, source_lengths: List[int]) -> Tuple[torch.Tensor, Tuple[torch.Tensor, torch.Tensor]]:
        ...
        last_hidden = last_hidden.view(2, 2, batch_size, self.hidden_size)
        last_cell = last_cell.view(2, 2, batch_size, self.hidden_size)

        # Compute dec_init_state = (init_decoder_hidden, init_decoder_cell)
        init_decoder_hidden_1 = self.h_projection_1(torch.cat((last_hidden[0][0], last_hidden[0][1]), dim=1))
        init_decoder_cell_1 = self.c_projection_1(torch.cat((last_cell[0][0], last_cell[0][1]), dim=1))
        init_decoder_hidden_2 = self.h_projection_2(torch.cat((last_hidden[1][0], last_hidden[1][1]), dim=1))
        init_decoder_cell_2 = self.c_projection_2(torch.cat((last_cell[1][0], last_cell[1][1]), dim=1))
    ...
    def decode(self, enc_hiddens: torch.Tensor, enc_masks: torch.Tensor,
        ...
        # enc_hiddens has dimension (b, src_len, h * 2)
        enc_hiddens_proj = self.att_projection(enc_hiddens)
        # construct Y with shape (tgt_len, b, e)
        Y = self.model_embeddings.target(target_padded)
        for Y_t in torch.split(Y, 1, dim=0):
            Y_t = Y_t.squeeze(0)
            Ybar_t = torch.cat((Y_t, o_prev), dim=1)
            dec_state_1, dec_state_2, o_t, _ = self.step(Ybar_t, dec_state_1, dec_state_2, enc_hiddens, enc_hiddens_proj, enc_masks)
            combined_outputs.append(o_t)
            o_prev = o_t

        # convert combined_outputs to single tensor shape (tgt_len, b, h)
    ...
    def step(self, Ybar_t: torch.Tensor,
            dec_state_1: Tuple[torch.Tensor, torch.Tensor],
            dec_state_2: Tuple[torch.Tensor, torch.Tensor],
        ...
        dec_state_1 = self.decoder_1(Ybar_t, dec_state_1)
        (dec_hidden_1, dec_cell_1) = dec_state_1
        dec_state_2 = self.decoder_2(dec_hidden_1, dec_state_2)
        (dec_hidden, dec_cell) = dec_state_2
        # compute the attention scores e_t, a Tensor shape (b, src_len)
        # dec_hidden is shape (b, h) and enc_hiddens_proj is shape (b, src_len, h)
        ...
        # alpha_t is shape (b, src_len) and enc_hiddens is shape (b, src_len, 2h)
        # we want a_t to be shape (b, 2h)
        a_t = torch.bmm(alpha_t.unsqueeze(1), enc_hiddens).squeeze(1)
        # dec_hidden is shape (b, h) and a_t is shape (b, 2h)
        # we want U_t to be shape (b, 3h) to be input for combined_output_projection
        ...
        combined_output = O_t
        return dec_state_1, dec_state_2, combined_output, e_t

...
if [ "$1" = "train" ]; then
	CUDA_VISIBLE_DEVICES=0 python run_beam.py train --src-lang=vi --tgt-lang=en --vocab=vocab.json --cuda --lr=7.5e-4 --patience=2 --valid-niter=200 --batch-size=64 --dropout=.2 --hidden-size=512 --embed-size=512
        ...
        CUDA_VISIBLE_DEVICES=0 python run_beam.py decode --src-lang=vi --tgt-lang=en --beam-size=10 model.bin test_outputs_beam_10.txt --cuda
	...
	python run_beam.py train --src-lang=vi --tgt-lang=en --vocab=vocab.json --lr=7.5e-4 --patience=2 --valid-niter=200 --batch-size=64 --dropout=.2 --hidden-size=512 --embed-size=512
        ...
        CUDA_VISIBLE_DEVICES=0 python run_beam.py translate --src-lang=vi --tgt-lang=en --beam-size=10 ./QA/trans_en_vi_queries_beam_10_dropped.txt model.bin ./QA/trans_vi_en_queries_beam_10.txt --cuda
elif [ "$1" = "translate_context" ]; then
        CUDA_VISIBLE_DEVICES=0 python run_beam.py translate --src-lang=vi --tgt-lang=en --beam-size=10 ./QA/trans_en_vi_context_beam_10_dropped.txt model.bin ./QA/trans_vi_en_context_beam_10.txt --cuda
elif [ "$1" = "vocab" ]; then
